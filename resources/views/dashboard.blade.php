...
<head>
    ...
    @if (file_exists(public_path('hot')) || file_exists(public_path('build/manifest.json')))
        {{-- Prefer Vite when available, but dashboard.css lives in resources/css/dashboard.css.
             If the Vite manifest doesn’t include it, fall back to inline CSS to avoid 500s. --}}
        @php
            $viteAsset = 'resources/css/dashboard.css';
            $viteManifest = file_exists(public_path('build/manifest.json'))
                ...
                : [];
            $viteHasAsset = is_array($viteManifest) && array_key_exists($viteAsset, $viteManifest);
        @endphp
        @if($viteHasAsset)
            @vite([$viteAsset])
        @else
