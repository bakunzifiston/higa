...
<head>
    ...
    <title>Edit User - HigaGroup</title>
    ...
    <main class="main">
        ...
        <section class="card">
            <div class="form-toolbar">
                <h3><i class="fas fa-user-edit" style="margin-right:6px;color:var(--brand-green-dark)"></i>Edit User: {{ $user->name }}</h3>
            ...
            <form method="POST" action="{{ route('users.update', $user) }}" class="form-grid">
                ...
                <div><label><i class="fas fa-user" style="margin-right:4px"></i> Full Name</label><input name="name" value="{{ old('name', $user->name) }}" required></div>
                <div><label><i class="fas fa-envelope" style="margin-right:4px"></i> Email Address</label><input name="email" type="email" value="{{ old('email', $user->email) }}" required></div>
                <div><label><i class="fas fa-lock" style="margin-right:4px"></i> New Password <span style="font-weight:400;color:var(--muted);font-size:0.85rem;">(leave blank)</span></label><input name="password" type="password" minlength="8"></div>
                <div><label><i class="fas fa-lock" style="margin-right:4px"></i> Confirm New Password</label><input name="password_confirmation" type="password"></div>
                <div class="full"><label><i class="fas fa-user-tag" style="margin-right:4px"></i> Role</label>
                    <select name="role_id" required>
                        <option value="1" {{ old('role_id', $user->role_id) == 1 ? 'selected' : '' }}>Admin</option>
                        <option value="2" {{ old('role_id', $user->role_id) == 2 ? 'selected' : '' }}>Collection Officer</option>
                        <option value="3" {{ old('role_id', $user->role_id) == 3 ? 'selected' : '' }}>Production Manager</option>
                        <option value="4" {{ old('role_id', $user->role_id) == 4 ? 'selected' : '' }}>Sales Team</option>
                    </select>
                </div>
                
                <div class="full actions">
                    <button class="primary" type="submit"><i class="fas fa-check"></i> Update User</button>
